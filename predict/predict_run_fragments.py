"""Run trained peptide classifiers on a precomputed table of sequence fragments."""
...
def predict_run(X):
    ...
    for model in models:
        ...
        # Store both the class decision and the positive-class probability.
...
def generate_fragments(sequence, name):
    """Generate unique subsequences between lengths 2 and 50 for one input sequence."""
    fragments = []
    fragment_names = []
    seen_fragments = set()  # Keep track of unique fragments
    count = 0
    for length in range(2, 51):  # Iterate over fragment lengths from 2 to 50
...
def read_fasta(file_path):
    """Read a FASTA file into a mapping from identifiers to sequences."""
    sequences = {}
...
def process_in_batches(fragments, model, tokenizer, pca, batch_size=100):
    """Convert input fragments into model-ready feature vectors in batches."""
    data = []
    count = 0 
    for start in range(0, len(fragments), batch_size):
        ...
        # Extract feature vectors in batches so large fragment tables remain tractable.
...
if __name__ == '__main__':
    # Read the fragment table, embed the sequences, and export prediction scores.
    names = ['unique_fragments']
    file = 'pd'
    for name in names:
        # name1 = f'result_{name}'
        # df = pd.read_excel(f"predict/{file}/{name1}.xlsx")
        df = pd.read_csv(f"predict/{file}/{name}.csv")


        file_path = f'predict/result/{file}/{name}'
        if not os.path.exists(file_path):
            # Create the folder if it does not exist
            os.makedirs(file_path)
        fragments = df['Seq'].values
        ...
        df = process_in_batches(fragments, model, tokenizer, pca, batch_size=100)
